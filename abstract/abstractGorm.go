...
type AbstractRepository[T Identifiable[K], K ID] interface {
	...
	// Find retrieves all entities of type T from the database.
	Find() ([]T, error)
	...
	// FindByKey retrieves all entities of type T by a specific field (key)
	// The `key` parameter specifies the field to search, and `value` is the value to match.
	...
	// The operation can optionally be executed within a transaction.
	UpdateSpecific(tx *gorm.DB, id K, specificFields map[string]any) error
...
// Find implements AbstractRepository.
func (repo *abstractRepositoryImpl[T, K]) Find() ([]T, error) {
	var entities []T
	var preloads []string
